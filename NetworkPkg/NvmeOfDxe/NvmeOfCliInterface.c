  ...
  )
{
  ...
  if (Found) {
    *Description = AllocateZeroPool (sizeof (gNvmeOfNbftList[Index].Device->ModelName));
    if (*Description == NULL) {
  ...
  )
{
  ...
  if (Flag == FALSE) {
    DEBUG ((DEBUG_ERROR, "NvmeOfCliDeleteMapKey: Error in removing MapKey\n"));
  ...
  )
{
  ...
  if (flags & SPDK_NVME_NS_DPS_PI_SUPPORTED) {
    ...
    Print (
      L"Protection Information Transferred as: %a\n",
  ...
  Print (
    L"Thin Provisioning:                     %a\n",
    nsdata->nsfeat.thin_prov ? "Supported" : "Not Supported"
    );
  Print (
    L"Per-NS Atomic Units:                   %a\n",
  ...
  Print (
    L"NGUID/EUI64 Never Reused:              %a\n",
    nsdata->nsfeat.guid_never_reused ? "Yes" : "No"
    );
  Print (L"Number of LBA Formats:                 %d\n", nsdata->nlbaf + 1);
  Print (
    L"Current LBA Format:                    LBA Format #%02d\n",
    nsdata->flbas.format
    );
  for (i = 0; i <= nsdata->nlbaf; i++) {
    Print (
      L"LBA Format #%02d: Data Size: %5d  Metadata Size: %5d\n",
      i,
  ...
  )
{
  ...
  Print (
    L"IEEE OUI Identifier:                   %02d %02d %02d\n",
    ReturnCdata->ieee[0],
    ReturnCdata->ieee[1],
    ReturnCdata->ieee[2]
    );
  ...
  Print (
    L"Doorbell Stride:                       %" PRIu64 " bytes\n",
    (uint64_t)1 << (2 + cap.bits.dstrd)
    );
  Print (
    L"NVM Subsystem Reset:                   %a\n",
  ...
  if (ReturnCdata->fwug == 0x00) {
    ...
    Print (
      L"Firmware Update Granularity:           %u KiB\n",
      ReturnCdata->fwug * 4
      );
  }

  Print (
    L"Per-Namespace SMART Log:               %a\n",
    ReturnCdata->lpa.ns_smart ? "Yes" : "No"
    );
  Print (
    L"Command Effects Log Page:              %a\n",
    ReturnCdata->lpa.celp ? "Supported" : "Not Supported"
    );
  Print (
    L"Get Log Page Extended Data:            %a\n",
    ReturnCdata->lpa.edlp ? "Supported" : "Not Supported"
    );
  Print (
    L"Telemetry Log Pages:                   %a\n",
  ...
  )
{
  ...
  for (Nsid = 1; Nsid <= NumOfNamespaces; Nsid++) {
    ...
    strncpy (MappingData->Key, Key, KEY_SIZE - 1);
    MappingData->Ioqpair = (VOID *)Device->qpair;
    MappingData->Nsid    = Device->NamespaceId;
    strncpy (MappingData->Traddr, Trid->traddr, ADDR_SIZE - 1);
    strncpy (MappingData->Subnqn, Trid->subnqn, NVMEOF_CLI_MAX_SIZE - 1);
  ...
  )
{
  ...
  if (ConnectData->IpMode == 0) {
    ...
    Status = AsciiStrToIpv6Address (ConnectData->LocalIp, &EndPointer, &AttemptData->SubsysConfigData.NvmeofSubsysHostIP.v6, NULL);
    if (EFI_ERROR (Status)) {
      Print (L"Invalid host IP address.\n");
      return EFI_INVALID_PARAMETER;
    }

    Status = AsciiStrToIpv6Address (ConnectData->Traddr, &EndPointer, &AttemptData->SubsysConfigData.NvmeofSubSystemIp.v6, NULL);
    if (EFI_ERROR (Status)) {
      Print (L"Invalid target IP address.\n");
      return EFI_INVALID_PARAMETER;
    }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Exit from protocol\n"));
